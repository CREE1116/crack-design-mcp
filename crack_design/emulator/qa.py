...
_OOC_RE = re.compile(r"^/\s*ooc\b", re.I)
...
def check(response: str, project: Project, session: Session, *,
    ...
    # /OOC halts the roleplay outright, so every rule that polices roleplay
    # form — narration, dialogue shape, images, length — has nothing to police.
    # Only the leak rules (meta / notation / impersonation) still apply.
    ooc = bool(_OOC_RE.match(user_input.strip())) or _suppresses_hud(shortcut)
    ...
    # ── 서술 형식 — OOC 모드에서는 전체 스킵 ────────────────────────
    for n, line in (body if narration_mark and not ooc else []):
        s = line.strip()
        if not s or _IMG.match(s) or dialogue_re.match(s) or s.startswith("[") or s.startswith("【"):
            continue
        # Contract-compliant document snippets, sounds, and placards wrapped in backticks
    ...
    for n, line in (body if not ooc else []):
        s = line.strip()
    ...
    # ── 이미지 규칙 — OOC 는 해설 모드라 이미지 계약 자체가 없음 ────
    if not ooc:
    ...
    if (length_min or length_max) and not ooc:
    ...
    # ── 분량 드리프트 ─────────────────────────────────────────────
    if baseline_chars and not (length_min or length_max) and not ooc:
...
def _check_images(response: str, project: Project, prev_response: str | None,
    ...
    if prev_response:
        ...
        if not seq and prev_last:
            # No images at all this turn, but previous turn had some → possible suppression
            out.append(Finding("image_suppressed", INFO,
                               "직전 응답에 이미지가 있었으나 이번 응답에는 전혀 없음 "
                               "— 감정 변화 시 재출력 규칙 확인 필요",
                               None, "", "# 8. 이미지 출력 규칙"))
