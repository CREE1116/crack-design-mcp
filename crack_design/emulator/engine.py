...
class Engine:
    ...
    def turn(self, session: Session, user_input: str, *, reply: str | None = None,
        ...
        # Duplicate turn guard: if client re-submits exact same (user_input, reply), prevent loop corruption
        if len(session.turns) >= 2 and reply is not None:
            last_u = session.turns[-2]
            last_a = session.turns[-1]
            if (last_u.role == "user" and last_u.content.strip() == user_input.strip() and
                    last_a.role == "assistant" and last_a.content.strip() == reply):
                dup_finding = qa.Finding(
                    "duplicate_turn_prevented", "warn",
                    "직전 턴과 100% 동일한 user_input 및 reply 가 재인입되어 중복 삽입이 차단되었습니다."
                )
                recalled = memory.select_recalled(session, self.cfg, window, user_input)
                return TurnResult(
                    session_id=session.id,
                    turn_index=sum(1 for t in session.turns if t.role == "user"),
                    ...
                    prompt_stats={
                        ...
                        "keyword_slots": len(prompt.activations),
                        "recalled_slots": len(recalled),
        ...
        self.log.append(session.id, {
            "session": session.id,
            "turn": sum(1 for t in session.turns if t.role == "user"),
        ...
        user_turn_number = sum(1 for t in session.turns if t.role == "user")
        return TurnResult(
            session_id=session.id,
