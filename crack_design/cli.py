...
from .emulator.sets import migrate, use
...
def _cfg(args) -> Config:
    ...
    overrides = {
        ...
        "memory.recalled_granularity": args.recall_granularity,
        "memory.recalled_max_chars": args.recall_max_chars,
        "keyword.priority": args.kb_priority,
...
def cmd_http(args) -> int:
    from .server.http_mcp import main as http_mcp_main
    return http_mcp_main(args.project, store=args.store, spec=args.spec,
                         provider=args.provider, variant=args.variant,
                         key_file=args.key_file, host=args.host, port=args.port,
                         endpoint=args.endpoint, auth_token=args.auth_token,
                         cors_origin=args.cors_origin)


def cmd_serve(args) -> int:
    from .server.webui import serve
...
def _add_global_flags(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--recall-granularity", choices=["turn", "sentence"],
                    help="lexical recall unit (sentence = fact-level, [EXTENSION])")
    ap.add_argument("--recall-max-chars", type=int,
                    help="per-slot char budget for <recalled_history>")
    ap.add_argument("--kb-priority", choices=["doc_order", "input_first"],
                    help="which entries survive the 3-slot cap")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = add("http", "run as a remote MCP server over HTTP")
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=8787)
    p.add_argument("--endpoint", default="/mcp")
    p.add_argument("--auth-token", help="Bearer token; defaults to CRACK_MCP_AUTH_TOKEN")
    p.add_argument("--cors-origin", default="*", help="CORS origin (prefer exact HTTPS origin)")
    p.add_argument("--key-file", help="provider API keys, outside repo")
